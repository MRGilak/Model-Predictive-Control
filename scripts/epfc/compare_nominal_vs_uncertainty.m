run(fullfile(fileparts(fileparts(fileparts(mfilename('fullpath')))), 'initDemo.m'));
clear; close all; clc;
...
%% Run Simulations
results_nominal = simulate_closed_loop(method, Ts, tf, N_model, mu, m, q, r, psi, ...
    noise_power, dist_amp, dist_time, dist_duration, ...
    u_min, u_max, x0, u0, Ref, beta, Ni, TOL, is_programmed_ref, @systems.model.epfc.update_nonlinear_state);

results_uncertain = simulate_closed_loop(method, Ts, tf, N_model, mu, m, q, r, psi, ...
    noise_power, dist_amp, dist_time, dist_duration, ...
    u_min, u_max, x0, u0, Ref, beta, Ni, TOL, is_programmed_ref, @systems.actual.epfc.update_nonlinear_state);
